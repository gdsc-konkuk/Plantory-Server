...
class PhotoNameGeneratorTest {
    ...
    @NullAndEmptySource
    @ValueSource(strings = [" "])
    fun `파일 이름이 없으면 예외 발생`(filename: String?) {
    ...
    fun `파일 확장자가 없는경우 예외 발생`(filename: String) {
        assertThatThrownBy { PhotoNameGenerator.of(filename) }
            .isInstanceOf(IllegalArgumentException::class.java)
    ...
    fun `이미지 파일 확장자가 아닌경우 예외 발생`(filename: String) {
        assertThatThrownBy { PhotoNameGenerator.of(filename) }
            .isInstanceOf(IllegalArgumentException::class.java)
            .hasMessage(
                "이미지 파일 확장자만 업로드 가능합니다. extension: " + filename.split("\\.".toRegex()).dropLastWhile { it.isEmpty() }
                    .toTypedArray()[1]
            )
    }
}
